/**
 * ACTIVE FLEET — Tekmetric Integration Page
 * Drop this file into: fleet-crm-frontend/src/pages/ActiveFleet.jsx
 * 
 * Uses your existing CRM's CSS — no extra styling needed.
 * Runs on demo data until you add your Tekmetric token in Settings.
 */
...
const DEMO_ROS = [
  ...
  { id:1013,rn:2205,cid:103,vid:206,sid:5,techId:1,saId:3,labor:4500, parts:2800, disc:0,   total:7300, paid:7300, created:'2025-11-10T09:00:00Z',updated:'2025-11-11T14:00:00Z',lastContact:'2025-11-10T09:30:00Z',contactMethod:'Call', jobs:[{name:'Oil Change',auth:true,labor:4500,parts:2800}] },
  { id:1014,rn:2180,cid:102,vid:204,sid:5,techId:2,saId:4,labor:4200, parts:2500, disc:0,   total:6700, paid:6700, created:'2025-10-05T10:00:00Z',updated:'2025-10-06T15:00:00Z',lastContact:'2025-10-05T10:30:00Z',contactMethod:'Text', jobs:[{name:'Oil & Filter Change',auth:true,labor:4200,parts:2500}] },
];
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Time in Status</th><th>Technician</th><th>Advisor</th>
                <th>Last Contact</th><th style={{textAlign:'right'}}>Value</th><th>Paid</th><th></th>
              </tr>
            </thead>
            <tbody>
              {filt.map(ro=>{
      ...
      {view==='due'&&(
        <div className="table-card">
          ...
          <div className="table-wrapper">
            <table>
              <thead>
                <tr><th>Vehicle</th><th>Company</th><th>Contact</th><th>Phone</th><th>Last Oil Change</th><th>Days Since</th><th>Carfax</th><th></th></tr>
              </thead>
              <tbody>
                {due.map(v=>(
                  <tr key={v.id} className={v.oil.st==='overdue'?'row-overdue':v.oil.st==='soon'?'row-today':''}>
                    ...
                    <td>
                      <button onClick={()=>togOil(v.id)} className="btn btn-ghost btn-sm" style={{fontSize:10.5}}>Not our customer</button>
                    </td>
                  </tr>
                ))}
                {due.length===0&&(
                  <tr><td colSpan={8}>
                    <div className="empty-state"><div className="icon">✅</div><div className="title">All caught up</div><div className="desc">No oil changes due</div></div>
                  </td></tr>
                )}
              </tbody>
            </table>
                ...
                <>
                    ...
                    <div style={{padding:'16px 20px'}}>
                      ...
                      {cfx&&(
                        <div style={{padding:'10px 14px',borderRadius:'var(--r-lg)',background:'var(--red-50)',border:'1px solid var(--red-100)',marginBottom:10}}>
                          ...
                          <div style={{fontSize:11,color:'var(--gray-400)',marginTop:2}}>Consider a follow-up call to win them back</div>
                        </div>
                      )}
                      ...
                      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                        ...
                        <button className="btn btn-danger btn-sm" style={{marginLeft:'auto'}}
                          onClick={()=>{ if(window.confirm(`Mark ${sv.year} ${sv.make} ${sv.model} as sold and remove from fleet?`)) markSold(sv.id); }}>
                          Mark as Sold
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="table-card">
                    <div className="table-card-header">
                      ...
                      <a href={tkVehLink(sv.id)} target="_blank" rel="noreferrer"
                        style={{marginLeft:'auto',color:'var(--blue-500)',fontSize:11,fontWeight:600,textDecoration:'none'}}>
                        Full history ↗
                      </a>
                    </div>
                        ...
                        <div key={ro.id} style={{borderBottom:'1px solid var(--gray-100)',padding:'12px 18px'}}>
                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8,marginBottom:8}}>
                            <div style={{display:'flex',alignItems:'center',gap:10}}>
                              ...
                              {ro.jobs.some(j=>j.auth&&isOil(j.name))&&<span className="badge badge-green" style={{fontSize:9.5}}>🛢 Oil Change</span>}
                            </div>
                            <div style={{display:'flex',alignItems:'center',gap:10}}>
                              <div style={{textAlign:'right'}}>
...
function SalesTab({ ros, companies, vehicles, employees, statuses }) {
  ...
  const byco = useMemo(()=>companies.map(c=>{
    ...
    return {...c,
      rev:p.reduce((s,r)=>s+r.total,0),labor:p.reduce((s,r)=>s+r.labor,0),
      parts:p.reduce((s,r)=>s+r.parts,0),open:cr.filter(r=>[1,2,3].includes(r.sid)).reduce((s,r)=>s+r.total,0),
      declined:dec.reduce((s,j)=>s+j.labor+j.parts,0),cnt:cr.length,
      avg:p.length?p.reduce((s,r)=>s+r.total,0)/p.length:0, allRos:cr,
    };
        ...
        <>
          ...
          <div className="table-card">
            ...
            <div className="table-wrapper">
              <table>
                <thead>
                  <tr><th>RO #</th><th>Vehicle</th><th>Status</th><th>Date</th><th>Tech</th><th>Advisor</th><th style={{textAlign:'right'}}>Labor</th><th style={{textAlign:'right'}}>Parts</th><th style={{textAlign:'right'}}>Total</th><th>Declined</th><th></th></tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={ro.id}>
                        ...
                        <td>
                          {dec.length>0?(
                            <div>
                              <span className="badge badge-overdue" style={{fontSize:9.5}}>{dec.length} job{dec.length>1?'s':''}</span>
                              <div style={{fontSize:10.5,color:'var(--red-500)',fontWeight:600}}>{f$(dec.reduce((s,j)=>s+j.labor+j.parts,0))}</div>
                            </div>
        ...
        <>
          <div className="stat-grid" style={{gridTemplateColumns:'repeat(5,1fr)'}}>
            {[{l:'Posted Revenue',v:f$(tot.rev),c:'green'},{l:'Labor',v:f$(tot.labor),c:''},{l:'Parts',v:f$(tot.parts),c:''},{l:'Open Value',v:f$(tot.open),c:'gold'},{l:'Declined',v:f$(tot.dec),c:tot.dec>0?'urgent':''}].map(s=>(
              <div key={s.l} className="stat-card">
                <div className="stat-label">{s.l}</div>
                <div className={`stat-value ${s.c}`} style={{fontSize:18}}>{s.v}</div>
              </div>
            ))}
          </div>
          ...
          <div className="table-card">
            ...
            <div className="table-wrapper">
              <table>
                <thead>
                  <tr><th>Account</th><th style={{textAlign:'right'}}>Revenue</th><th style={{textAlign:'right'}}>Labor</th><th style={{textAlign:'right'}}>Parts</th><th style={{textAlign:'right'}}>Avg Ticket</th><th style={{textAlign:'right'}}>Open</th><th style={{textAlign:'right'}}>Declined</th><th style={{textAlign:'right'}}>ROs</th><th></th></tr>
                </thead>
                <tbody>
                  {byco.map(co=>(
                    <tr key={co.id} onClick={()=>setSelCo(co.id)} style={{cursor:'pointer'}}>
                      <td>
                        <div style={{fontWeight:600}}>{co.name}</div>
                        <div style={{marginTop:5,height:3,background:'var(--gray-100)',borderRadius:3,width:120,overflow:'hidden'}}>
                          <div style={{height:'100%',width:`${(co.rev/mx)*100}%`,background:'var(--green-500)',borderRadius:3}}/>
                        </div>
                      </td>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,alignItems:'start'}}>
      <div style={{display:'flex',flexDirection:'column',gap:14}}>
        ...
        <div className="table-card" style={{padding:18}}>
          ...
          <div className="form-group" style={{marginBottom:0}}>
            ...
            <div style={{fontSize:11,color:'var(--gray-400)',marginTop:5}}>
              {poll===1?'⚡ Max freshness':poll<=5?'✅ Recommended':'🔵 Background — low API usage'}
            </div>
          </div>
        </div>
        ...
        <div className="table-card" style={{padding:18}}>
          ...
          <div style={{marginTop:10,fontSize:11.5,color:'var(--gray-500)',padding:'7px 10px',background:'var(--gray-50)',borderRadius:'var(--r-md)',border:'1px solid var(--gray-200)'}}>
            Flagging at <strong>{oilInterval}+ days</strong>. Due Soon = within 14 days of threshold.
          </div>
        </div>
        ...
        <div className="table-card" style={{padding:18}}>
          ...
          <div style={{fontSize:12,color:'var(--gray-400)',marginBottom:14}}>
            Alerts you when a fleet vehicle is serviced at a competitor. Requires a Carfax for Dealers account.
          </div>
          <div className="form-group" style={{marginBottom:10}}>
            ...
            <input
              ...
              placeholder={cfxEnabled ? 'Paste your Carfax API key here' : 'Enable Carfax above to enter your key'}
              disabled={!cfxEnabled}
              style={{opacity:cfxEnabled?1:0.5}}
            />
          </div>
          {cfxEnabled && cfxKey && (
            <div style={{padding:'8px 12px',background:'var(--green-50)',border:'1px solid var(--green-100)',borderRadius:'var(--r-md)',fontSize:12,color:'var(--green-600)',fontWeight:600}}>
              ✓ Carfax connected — competitor service alerts active
            </div>
          )}
          {cfxEnabled && !cfxKey && (
            <div style={{padding:'8px 12px',background:'#fffbeb',border:'1px solid var(--yellow-100)',borderRadius:'var(--r-md)',fontSize:12,color:'var(--yellow-500)',fontWeight:600}}>
              ⚠ Enter your API key above to activate alerts
            </div>
          )}
          {!cfxEnabled && (
            <div style={{fontSize:11,color:'var(--gray-400)'}}>
              No Carfax dealer account yet?{' '}
              <a href="https://www.carfax.com/dealer/" target="_blank" rel="noreferrer"
...
export default function ActiveFleet() {
  ...
  const [isDemo,      setIsDemo]      = useState(true); // flips to false once token is saved

  // Data state — starts as demo, replaced by live data once connected
  ...
  return (
    <>
      {/* Page header — matches your existing CRM exactly */}
      <div className="page-header">
        ...
        <div style={{display:'flex',gap:6,alignItems:'center'}}>
          {activeRos.length>0 && <span className="badge badge-gold">{activeRos.length} Open ROs</span>}
          {idleROs.length>0   && <span className="badge badge-overdue">⏱ {idleROs.length} Idle</span>}
        </div>

        <div className="header-actions">
          <span style={{fontSize:11.5,color:'var(--gray-400)'}}>
            Synced {lastSync.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'})}
          </span>
          <button onClick={doSync} className="btn btn-ghost btn-sm" disabled={syncing}>
