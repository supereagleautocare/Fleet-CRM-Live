      ...
      <div className="page-header">
        ...
        <div className="header-actions">
          <div style={{display:'flex',gap:4,background:'var(--gray-100)',borderRadius:8,padding:3}}>
            <button className={`btn btn-sm ${!showAll?'btn-navy':'btn-ghost'}`} style={{border:'none'}} onClick={()=>setShowAll(false)}>Due Today</button>
            <button className={`btn btn-sm ${showAll?'btn-navy':'btn-ghost'}`} style={{border:'none'}} onClick={()=>setShowAll(true)}>All Scheduled</button>
          </div>
        </div>
      </div>
